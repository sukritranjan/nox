...
frac_no3m=0.8 #Fraction of NOx that is delivered as NO3- (vs. NO2-)
...
###Photochemical reduction of NOx
k0_min=0.6e-6 #s**-1; Scaled to 0C from median value for modern seawater from Zafiriou+1979a. 
k0_max=2.4e-6 #s**-1; Scaled to 50C from median value for modern seawater from Zafiriou+1979a.
...
k1_min=1.1e-8 #s**-1;  Scaled to 0C from median value for modern seawater from Zafiriou+1979b.
k1_max=4.6e-8 #s**-1;  Scaled to 50C from median value for modern seawater from Zafiriou+1979b.
...
k3_min=8.e-7 #M**-1 s**-1; Petersen+1979, Ottley+1997
...
def reac3_no3m_fe(k, conc_NO3m, conc_Fe2p):
	...
	"""
	k=np.ones(np.shape(conc_NO3m))*k
	k[conc_NO3m>conc_Fe2p]*=0.1
...
colors=np.array(['green','blue']) #'darkgreen', 'indigo', 'orange', 'brown', 
hatch_list=np.array(['//','\\'])
label_list=np.array(['NOx- + 10uM Fe(II)-->N2O', 'NOx- + 600uM Fe(II)-->N2O'])

##Nitrite Plot
ax0.set_title('NO2-')

#Supply
ax0.plot(conc_no2m_range, supply_rate_no2_low, color='black', linestyle='--', linewidth=2, label='NOx Prod. (Low)')
ax0.plot(conc_no2m_range, supply_rate_no2_high, color='black', linestyle='-', linewidth=2, label='NOx Prod. (High)')

#Photochemistry
ax0.fill_between(conc_no2m_range, loss_rates_no2m[0, 0, :, 0], loss_rates_no2m[0, 1, :, 0], facecolor='indigo', alpha=0.6, edgecolor='black', label='Photoreduction')
...
#Other reduction
ax0.plot(conc_no2m_range, loss_rates_no2m[2, 0, :, 0], color='brown', linestyle='--', label='NO2- + Fe(II)-->NH3')
ax0.plot(conc_no2m_range, loss_rates_no2m[3, 0, :, 0], color='orange', linestyle='--', label='Anammox')

#Vents
ax0.fill_between(conc_no2m_range, loss_rates_no2m[4, 0, :, 0], loss_rates_no2m[4, 1, :, 0], facecolor='red', alpha=0.6, edgecolor='black', label='Vents')
...
#Photochemistry
ax1.fill_between(conc_no3m_range, loss_rates_no3m[0, 0, :, 0], loss_rates_no3m[0, 1, :, 0], facecolor='indigo', alpha=0.6, edgecolor='black')
...
#Vents
ax1.fill_between(conc_no3m_range, loss_rates_no3m[2, 0, :, 0], loss_rates_no3m[2, 1, :, 0], facecolor='red', alpha=0.6, label='Vents')
...
ax0.legend(bbox_to_anchor=[-0.09, 1.08, 1.08, .15],loc='lower left', ncol=3, mode='expand', borderaxespad=0., fontsize=12)
plt.tight_layout(rect=(0,0,1,0.88))
plt.subplots_adjust(wspace=0., hspace=0.2)

plt.savefig('./Plots/important_rxns_ocean_focused_v5.pdf', orientation='portrait',papertype='letter', format='pdf')
...
ax0.plot(conc_no2m_range, supply_rate_no2_low, color='black', linestyle='--', linewidth=2, label='NOx Prod. (Low)')
ax0.plot(conc_no2m_range, supply_rate_no2_high, color='black', linestyle='-', linewidth=2, label='NOx Prod. (High)')
ax0.plot(conc_no2m_range, supply_rate_no2_low_catchmentarea, color='hotpink', linestyle='--', linewidth=2, label='NOx Prod. \n(Low, DR=100)')
ax0.plot(conc_no2m_range, supply_rate_no2_high_catchmentarea, color='hotpink', linestyle='-', linewidth=2, label='NOx Prod. \n(High, DR=100)')
...
plt.savefig('./Plots/important_rxns_pond_focused_v5.pdf', orientation='portrait',papertype='letter', format='pdf')
...
nox_input_list=10.**np.linspace(np.log10(nox_input_low), np.log10(nox_input_high), num=10) #cm**-2 s**-1; flux of NOx from atmosphere.
...
plt.savefig('./Plots/conc_system_v5.pdf', orientation='portrait',papertype='letter', format='pdf')
